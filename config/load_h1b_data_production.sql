-- =============================================================================
-- H1B LCA Data Loader + Derived Table Population (Production - api schema)
-- =============================================================================
-- This script:
--   1. Loads h1b.csv into api.h1b_lca_data (source table)
--   2. Populates all derived tables (pre-computed aggregations)
--
-- USAGE (DigitalOcean App Platform - runs as PRE_DEPLOY job):
...
-- api.h1b_lca_stats: Aggregate statistics (1 row)
TRUNCATE api.h1b_lca_stats;
INSERT INTO api.h1b_lca_stats
  SELECT
...
-- api.h1b_distinct_employers: Unique employers with filing counts
TRUNCATE api.h1b_distinct_employers;
INSERT INTO api.h1b_distinct_employers
...
-- api.h1b_distinct_jobs: Unique job titles per employer
TRUNCATE api.h1b_distinct_jobs;
INSERT INTO api.h1b_distinct_jobs
  SELECT UPPER(TRIM(employer_name)) as employer_name,
...
-- api.h1b_distinct_locations: Unique locations per employer
TRUNCATE api.h1b_distinct_locations;
INSERT INTO api.h1b_distinct_locations
  SELECT UPPER(TRIM(employer_name)) as employer_name,
...
-- api.h1b_state_stats: State-level aggregation
TRUNCATE api.h1b_state_stats;
INSERT INTO api.h1b_state_stats
  WITH extracted AS (
    SELECT
...
-- STEP 3: Verify
-- =============================================
SELECT 'h1b_lca_data' as table_name, COUNT(*) as row_count FROM api.h1b_lca_data
UNION ALL
SELECT 'h1b_lca_stats', COUNT(*) FROM api.h1b_lca_stats
UNION ALL
SELECT 'h1b_distinct_employers', COUNT(*) FROM api.h1b_distinct_employers
UNION ALL
SELECT 'h1b_distinct_jobs', COUNT(*) FROM api.h1b_distinct_jobs
UNION ALL
SELECT 'h1b_distinct_locations', COUNT(*) FROM api.h1b_distinct_locations
UNION ALL
SELECT 'h1b_state_stats', COUNT(*) FROM api.h1b_state_stats;
